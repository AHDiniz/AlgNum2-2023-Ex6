function example1()
    ...
    for i = 1 : numel(ns)
        ...
        errors_explicit = [errors_explicit; sqrt(sum((u_e - sol_values_ex) .^ 2)) / sqrt(sum(sol_values_ex .^ 2))];
        errors_implicit = [errors_implicit; sqrt(sum((u_i - sol_values_im) .^ 2)) / sqrt(sum(sol_values_im .^ 2))];
